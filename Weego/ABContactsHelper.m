...
+ (NSArray *) contactsMatchingName: (NSString *) fname
{
	...
	pred = [NSPredicate predicateWithFormat:@"contactName contains[cd] %@ OR lastname contains[cd] %@ OR nickname contains[cd] %@ OR middlename contains[cd] %@", fname, fname, fname, fname];
...
+ (NSArray *) contactsMatchingEmail: (NSString *) email
{
    ...
    NSArray *contactsWithEmail = [contacts filteredArrayUsingPredicate:pred];
    pred = [NSPredicate predicateWithFormat:@"emailaddresses contains[cd] %@", email];
    return [contactsWithEmail filteredArrayUsingPredicate:pred];
}
