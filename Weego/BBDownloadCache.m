...
- (void)updateExpiryForRequest:(SGASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-SGASIHTTPRequest-Expires"];
...
- (NSDate *)expiryDateForRequest:(SGASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [SGASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
- (void)storeResponseForRequest:(SGASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & SGASIDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
	...
	// Create a special 'X-SGASIHTTPRequest-Expires' header
	...
	// We store this as a timestamp to make reading it eSGASIer as NSDateFormatter is quite expensive
    
	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-SGASIHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-SGASIHTTPRequest-Response-Status-Code"];
...
- (BOOL)isCachedDataCurrentForRequest:(SGASIHTTPRequest *)request
{
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-SGASIHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-SGASIHTTPRequest-Expires"];
		if (expires) {
